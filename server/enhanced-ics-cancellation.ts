 ...
 * 
 * This module creates RFC 5546 compliant cancellation ICS files from original event ICS data.
 * It maintains the exact same UID and properly preserves resource information.
 */
...
export function generateCancellationIcs(originalIcs: string, eventData: EventData): string {
  ...
  if (hasUidLine) {
    ...
    cancellationLines.push(uidLine);
  } else {
