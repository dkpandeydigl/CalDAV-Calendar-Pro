/**
 * Script to create a test event with attendees
 * 
...
(async function() {
  ...
  const now = new Date();
  const eventTitle = `Attendee Test V2 ${now.toISOString().split('T')[0]} ${now.getHours()}:${now.getMinutes()}`;
  ...
  const createData = {
    title: eventTitle,
    description: 'Test event for attendee preservation testing V2',
  ...
  try {
    ...
    console.log('EVENT ID:', createdEvent.event.id);
    
    return createdEvent.event.id;
  } catch (error) {
